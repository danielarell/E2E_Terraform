...
def download_model_from_s3():
    ...
    if BUCKET_NAME:
        try:
            logger.info(f"Intentando descargar modelo desde s3://{BUCKET_NAME}/{MODEL_S3_KEY}")
            s3 = boto3.client('s3')
            s3.download_file(BUCKET_NAME, MODEL_S3_KEY, MODEL_LOCAL_PATH)
            logger.info("Modelo descargado exitosamente.")
        except Exception as e:
            logger.error(f"Error descargando de S3: {e}. Se intentará usar archivo local.")
    else:
        logger.warning("S3_BUCKET no configurado. Se usará el modelo local si existe.")

# Intentar obtener el modelo al arrancar
download_model_from_s3()

# Cargar el modelo (el Pipeline de la Persona 1 ya incluye el StandardScaler)
try:
...
class PredictionRequest(BaseModel):
    # El orden debe ser: [MedInc, HouseAge, AveRooms, AveBedrms, Population, AveOccup, Latitude, Longitude]
    features: List[float] = Field(
        ..., 
        min_items=8, 
        max_items=8,
        example=[8.32, 41.0, 6.98, 1.02, 322.0, 2.55, 37.88, -122.23]
    )
...
def predict(request: PredictionRequest):
    ...
    try:
        # El modelo de la Persona 1 espera una lista de listas o array 2D
